...
then
  OBJECT_KEY_BASEPATH="screenshots/${PR_NUMBER}-${GIT_SHORT_COMMIT}"
else
  OBJECT_KEY_BASEPATH="screenshots/${TRAVIS_BRANCH}"
fi
...
then
  ...
  do
    BASENAME=$(echo ${file} | sed 's|^screenshots/||')
    BASE_BRANCH_OBJECT_KEY="screenshots/${TRAVIS_BRANCH}/${BASENAME}"
...
EOF
  ...
  <summary>E2E screenshots differences between '**PR${PR_NUMBER}-${GIT_SHORT_COMMIT}**' and base branch '**${TRAVIS_BRANCH}**'</summary>\n\n
$(cat /tmp/pr-differences-urls | while read line; do echo "\\n\\n  $line\\n\\n"; done)\n\n
</details>
EOF
  cat >> /tmp/pr-comment.json <<EOF
<details>\n
  <summary>Complete E2E screenshots collection for 'PR${PR_NUMBER}-${GIT_SHORT_COMMIT}'</summary>\n\n
